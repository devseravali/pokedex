...
export const usePokemons = (limit = 10) => {
    ...
    const fetchPokemons = async (loadMore = false) => {
        ...
        try {
            ...
            const fullPokemons = await Promise.all(summaries.map((p) => getPokemonWithCache(p.name)));
            setPokemons((prev) => loadMore
                ? [
                    ...prev,
                    ...fullPokemons.filter((p) => !prev.some((existing) => existing.id === p.id)),
                ]
                : fullPokemons);
