...
export async function getPokemon(name) {
    const rawData = await fetchData(`https://pokeapi.co/api/v2/pokemon/${name}`);
    return {
...
export const getPokemonWithCache = async (name) => {
...
export async function getAllPokemonsWithCache(limit = 10, offset = 0) {
...
export async function buscarPokemon(nome, setLoading, setPokemons, setError) {
    setLoading(true);
    try {
...
export async function buscarPokemonsPorTipo(tipo) {
