import { fetchData, getPokemon, getPokemonWithCache, getAllPokemonsWithCache, fetchTypesWithCache, getFavoritePokemons, FavoritePokemon, mergeUniquePokemons, buscarPokemon, buscarPokemonPorNome, buscarPokemonsPorTipo } from '../services/api';
describe('API Service', () => {
    ...
    it('mergeUniquePokemons mescla sem duplicar', () => {
        ...
        const merged = mergeUniquePokemons(prev, next);
    ...
    it('buscarPokemonsPorTipo retorna lista filtrada', async () => {
        global.fetch.mockResolvedValue({
            ok: true,
            json: () => Promise.resolve({ pokemon: [{ pokemon: { name: 'pikachu', url: 'url' } }] })
        });
